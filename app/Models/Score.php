...
class Score extends Model
{
    ...
    static public function createMeetingScores($input) {
        ...
        foreach ($input as $key => $value) {
            ...
            if (isset($pointsData[$new_key]) && $pointsData[$new_key]->point_value == 0) {
                dd($key);
            }
            ...
            foreach (config('constants.meeting_form_ids_array') as $form_id => $slug) {
                if ($key == $form_id) {
                    ...
                    foreach ($ids as $id) {
                        $newScore = new Score($input);
                        $newScore->created_by = Auth::user()->id;
                        $newScore->which_half = $meeting_half;
                        $newScore->user_id = $id;
                        $newScore->point_id = $pointsData[$slug]->id;
                        $newScore->point_value = $pointsData[$slug]->point_value;
                        $newScore->save();
                    }
                }
            }
            ...
            // Save score if it hasn't been saved
